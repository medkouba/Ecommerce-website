...
const GetListCategories=async()=>{
  ...
  ScategorieService.fetchScategoriesByID(idcat)
  .then((res) => {
   setScategories(res.data);
  });
  }
  const GetListSCategorie=async()=>{
    //await Axios.get("/scategories/cat/"+idcat)
  ...
  //await Axios.post("/articles", objetarticle)
...
onChange={(event)=>{setcategorieID(event.target.value); GetListSCategories(event.target.value) }}
